...
// rewriteLinks walks every markdown inline/reference link and image in `body`
// and rewrites its target via resolveLink. Covers [text](target) and
// ![alt](target); anchors and code spans are left alone by the regex shape.
function rewriteLinks(body, srcAbsPath) {
  ...
  return body.replace(/(!?)\[([^\]]*)\]\(([^)\s]+)(\s+"[^"]*")?\)/g, (m, bang, label, target, title) => {
...
async function run() {
  ...
  console.log(`sync-docs: projected ${DOCS.length} docs into ${path.relative(repoRoot, outDir)}/`);
}
