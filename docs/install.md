...
- **`just docker`** — the lean control/broker image carrying `converge` + `conctl`
  on an alpine base. This is the tier that talks to Postgres and serves the API/UI;
  it carries **no** provider code.
- **`just docker-stdworker`** — the default worker image (`Dockerfile.stdworker`):
...
```sh
just docker                                 # control/broker → converge:<git-describe>
just docker image=myrepo/converge:v1        # custom tag
just docker-stdworker                       # worker → converge-stdworker:latest
just docker-stdworker image=myrepo/stdworker:v1
```
...
The chart runs control + broker by default; opt the generic worker in with
`--set stdWorker.enabled=true` and point it at the **worker** image
(`--set stdWorker.image.repository=<registry>/converge-stdworker`, built by
`just docker-stdworker`) — that tier needs the heavier tooling image, not the lean
control/broker one. By default it advertises **all** the std\* kinds; narrow a pool
with `--set stdWorker.kinds=stdterraform,stdshell`.
