...
see [the features poster](assets/features.png).)

| Axis | Crossplane / kro | Converge | Why it's an edge |
|---|---|---|---|
| **Scale** | Bounded by etcd (~8 GB, thousands of objects) and one controller per graph | Tuned for **millions** of resources on one Postgres primary; **>4,000 reconciliations/sec** | The whole graph lives in the database and a reconcile is a single commit, not a controller wakeup — so it scales with the database, not etcd |
