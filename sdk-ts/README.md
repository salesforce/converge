...
## Consuming it (no public npm)

This package is not on the public npm registry. A worker project pulls it in one of three
ways — all of which build it automatically (the `prepare` script runs on install):

1. **Git dependency** — point directly at the repo:
   ```json
   { "dependencies": { "@converge/worker-sdk": "github:salesforce/converge#main" } }
   ```
   npm clones, runs `prepare` (builds `dist/`), and installs it. Requires the repo's
   `sdk-ts/` to be the package root (use a subdirectory/monorepo git-dep syntax if your
   client tooling supports it, or a tarball — below).

2. **Tarball** — build an artifact and hand it out / host it anywhere:
   ```sh
   cd sdk-ts && npm pack        # → converge-worker-sdk-0.1.0.tgz (dist/ + README + package.json)
   # in the consumer:
   npm install /path/to/converge-worker-sdk-0.1.0.tgz
   ```

3. **Private registry** — publish to an internal Artifactory / GitHub Packages registry:
   ```sh
   cd sdk-ts && npm publish --registry=https://your-registry
   # consumer sets that registry for the @converge scope in .npmrc, then `npm install`.
   ```

In all three the consumer imports the same way:
