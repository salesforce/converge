...
# ── config (these are VARIABLES — override with an assignment that PRECEDES the
#    recipe, e.g. `just image=myrepo/x:v1 docker`; `just docker image=…` would
#    read `image=…` as a second recipe name and fail) ─────────────────────────
...
# flags an uncommitted working tree. `version` is a VARIABLE, so override it with
# an assignment BEFORE the recipe: `just version=… build`.
...
#
# Build the linux/amd64 control/broker image (host-built binaries). The tag is the
# `image` VARIABLE, so override it with an assignment BEFORE the recipe:
# `just image=myrepo/x:v1 docker`.
[doc("build the linux/amd64 control/broker container image (override tag: `just image=myrepo/x:v1 docker`)")]
docker: ui
...
# one image runs any std* kind (stdio / stdshell / stdterraform). The tag is a
# positional recipe ARGUMENT, so pass it after the recipe: `just docker-stdworker
# myrepo/stdworker:v1`. The stdterraform demo builds this same image.
[doc("build the linux/amd64 stdworker image (override tag: `just docker-stdworker myrepo/stdworker:v1`)")]
...
# Build the SHIPPED binaries (converge + conctl + stdworker) for every released
# platform and package them into per-platform tarballs under dist/, with a
# SHA256SUMS manifest — the artifacts the release workflow attaches to a GitHub
# Release. Matrix: {linux,darwin} × {amd64,arm64} = 4 platforms × 3 binaries.
#
# The build is a pure-Go cross-compile (CGO_ENABLED=0), so one host builds every
# target with no C toolchain. converge embeds the UI (ui/embed.go // go:embed
# dist/*) at Go-compile time, so `ui` runs ONCE up front and every converge binary
# carries the same SPA. Flags mirror the `docker` recipe (-trimpath, -s -w, the
# BuildVersion stamp) so a downloaded binary and an image binary are identical.
#
# Layout (GoReleaser-style): one archive per platform holds all three binaries, so
# a user grabs a single file for their OS/arch. SHA256SUMS lets them verify.
[doc("build + package the release binaries (converge/conctl/stdworker × linux/darwin × amd64/arm64) into dist/")]
release-binaries: ui
    #!/usr/bin/env bash
    set -euo pipefail
    VER='{{version}}'
    ...
    # The shipped binaries as "name:package" pairs and the platform matrix as
    # "os/arch" — plain indexed arrays (NO associative arrays), so this runs on
    # bash 3.2 (stock macOS) as well as the CI runner's bash 5. worker (classic
    # demo) is NOT shipped — only these three.
    builds="converge:./cmd/converge conctl:./cmd/conctl stdworker:./cmd/stdworker"
    platforms="linux/amd64 linux/arm64 darwin/amd64 darwin/arm64"
    for plat in $platforms; do
      os="${plat%/*}"; arch="${plat#*/}"
      stage="$OUT/converge_${VER}_${os}_${arch}"
      mkdir -p "$stage"
      for pair in $builds; do
        name="${pair%%:*}"; pkg="${pair#*:}"
        echo "building $name for $os/$arch"
        CGO_ENABLED=0 GOOS="$os" GOARCH="$arch" \
          go build -trimpath -ldflags="-s -w -X main.BuildVersion=${VER}" \
          -o "$stage/$name" "$pkg"
      done
      # One tarball per platform, containing the three binaries (no nested dir).
      tar -czf "$OUT/converge_${VER}_${os}_${arch}.tar.gz" -C "$stage" .
      rm -rf "$stage"
    done
    # Checksums over the archives, for `shasum -a 256 -c` verification on download.
    ( cd "$OUT" && shasum -a 256 *.tar.gz > SHA256SUMS )
    echo "── dist/ ──"; ls -1 "$OUT"

# Remove build outputs.
[doc("remove build outputs (bin/ + ui/dist/ + dist/)")]
clean:
...
# Run the UNIT tests under the Go race detector. Docker-free (excludes ./test, whose
# integration suite needs a Postgres testcontainer + is gated separately in `just test`),
# so it's a fast concurrency gate over the packages that carry real shared-memory
# concurrency: the API schema surface (atomic snapshot vs. background NOTIFY refresher),
# the SDK worker runner (in-flight/result-buffer under a stream drop), the broker fanout
# mesh, and the runtime dispatcher/reactor. Wired into CI's fast lane. A race here fails
# the build at the boundary — the class of bug (unsynchronized field access) that a plain
# `go test` silently passes.
[doc("run the unit tests under the race detector (Docker-free concurrency gate)")]
race:
    go test -race -count=1 ./internal/... ./sdk-go/...
