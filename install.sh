...
if [ "$VERSION" = "latest" ]; then
  ...
  if [ -z "$VERSION" ]; then
    ...
    echo "  (check network access to api.github.com, or pass CONVERGE_VERSION=vX.Y.Z)" >&2
    exit 1
  fi
fi
...
tmp="$(mktemp -d)"
trap 'rm -rf "$tmp"' EXIT
# Release assets are downloaded from the per-tag download URL (the optional auth
# header carries through if one was set).
base="https://github.com/${REPO}/releases/download/${VERSION}"
echo "downloading ${tarball}…"
curl_dl -o "${tmp}/${tarball}"  "${base}/${tarball}"
curl_dl -o "${tmp}/SHA256SUMS"  "${base}/SHA256SUMS"
