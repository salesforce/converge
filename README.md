...
or any external program. See [how Converge compares](docs/how-converge-compares.md) to
...
any-language workers dial to pull work ([topology](docs/assets/topology.svg)). For a
one-picture overview, see [Converge features at a glance](docs/assets/features.png).
...
Then open the UI at http://localhost:8080 and watch a BOM fan out into accounts/VPCs/routes/TGW and converge to Ready. `Ctrl+C` tears it all down. Seven demos cover Go, CEL, Starlark, Terraform, any-language `stdio`, TypeScript, and GitOps sync — see [Demos](#demos) below.
...
Seven runnable demos, each with a `just demo` and a walkthrough in its README ([the Quickstart](#quickstart) shows the flow):
...
`just build` produces three shipped binaries in `bin/` (plus each demo's own worker):
