...
func serveProbes(ctx context.Context, addr string) {
	...
	// Shut the listener down when the worker's ctx ends, so HEALTH_ADDR is released.
	// For a standalone worker the OS would reclaim the socket at exit anyway, but an
	// EMBEDDED worker (a supported use — Serve embeds cleanly, never os.Exits) whose
	// ctx is cancelled must free the port so a subsequent Serve on the same address
	// isn't blocked by the old listener still bound.
	go func() {
		<-ctx.Done()
