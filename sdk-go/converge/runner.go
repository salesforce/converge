...
// untrack removes a completed task from the in-flight set (called after its result is
// enqueued/buffered). The buffered result, if any, is what carries the completion forward;
// the in-flight entry's job is done.
...
func (r *runner) send(_ context.Context, t *workerpb.StageTask, sc *workerpb.StageComplete) {
	...
	// Mark done so the attestation ticker stops attesting it (the result now carries the
	// completion forward), and remove it from the in-flight set.
	r.untrack(token)
