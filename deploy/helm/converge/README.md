...
The chart defaults `image.repository` to the published `ghcr.io/salesforce/converge`
(and `stdWorker.image.repository` to `ghcr.io/salesforce/converge-stdworker`), with
the tag pinned to `latest` (`pullPolicy: Always`, so pods re-pull the rolling tag) —
so the minimum install only needs a DSN. Pin `image.tag` to a `vX.Y.Z` or set
`image.digest` for a reproducible deploy; override the repository to use your own
registry/mirror.
...
just image=converge:latest docker                                  # lean control/broker image (image= is a variable → precedes the recipe)
just docker-stdworker converge-stdworker:latest                    # worker image (tofu + cloud CLIs; the tag is a positional recipe argument)
...
> `just docker …` builds and the pods fail `ErrImageNeverPull`. Note this recipe
> OVERRIDES `image.repository` to the local `converge` / `converge-stdworker` tags (the
> chart default is the remote `ghcr.io/salesforce/converge*`, which `Never` would refuse
> to pull). This recipe is for local testing only — for real deployments use `database.existingSecret`
