...
func TestQuarantine(t *testing.T) {
	...
	t.Run("frozen_from_operate", func(t *testing.T) {
		// Invoking a subresource verb on a quarantined resource must FAIL CLOSED:
		// EnqueueOperationWork's freeze guard selects 0 rows, so CreateOperationRow
		// returns ErrOperationNotEnqueued and rolls the whole tx back — leaving
		// NEITHER a work_queue 'operate' row NOR an orphaned resource_operations row
		// (both writes are in one tx). A silent success with no backing task (the
		// old behavior) would strand a 'pending' operation forever. (account has the
		// enable_flow_logs verb on networking; here we drive CreateOperationRow
		// directly and assert both the work_queue and the operations side.)
		count := func(query string) int {
			var n int
			require.NoError(t, pool.QueryRow(ctx, query, childID).Scan(&n))
			return n
		}
		_, err := st.CreateOperationRow(ctx, childID, "noop-verb", []byte(`{}`), "test")
		require.ErrorIs(t, err, store.ErrOperationNotEnqueued,
			"invoking a verb on a quarantined resource must fail closed, not silently succeed")
		require.Zero(t, count(`SELECT count(*) FROM work_queue WHERE resource_id=$1 AND task_type='operate'`),
			"no operate task enqueued for a quarantined resource (freeze guard)")
		require.Zero(t, count(`SELECT count(*) FROM resource_operations WHERE resource_id=$1`),
			"no orphaned operation row left behind (tx rolled back)")
	})
