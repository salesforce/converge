...
func (s *Server) invokeOperation(ctx context.Context, in *invokeOpInput) (*invokeOpOutput, error) {
	...
	if err != nil {
		// The enqueue no-opped: the resource was frozen, or an operate task was
		// already queued, in the window since the checks above (a TOCTOU race —
		// a concurrent quarantine, or a second verb submitted while one is still
		// queued). The store rolled the operation row back; surface 409, not 500.
		if errors.Is(err, store.ErrOperationNotEnqueued) {
			return nil, huma.Error409Conflict("cannot invoke verb: the resource was set aside or a verb is already queued; retry")
		}
