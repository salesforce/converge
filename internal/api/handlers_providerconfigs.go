...
type applyProviderConfigInput struct {
	Body struct {
		...
		// providerconfigs.data. There is no SEPARATE cap on the decoded bundle: the
		// whole providerconfig apply is bounded by configBody (maxProviderConfigApplyBytes,
		// 50 MiB) at ingress, so the bundle is at most ~37 MiB decoded. Empty leaves the
		// bundle empty. Kind-wide logic, so it only makes sense on a DEFAULT config (no
		// per-resource override).
		Data string `json:"data,omitempty" doc:"base64-encoded opaque provider bundle (e.g. a zip of Starlark .star files); decoded into the config's data bytes. Bounded by the 50 MiB request-body cap (~37 MiB decoded)."`
	}
}
