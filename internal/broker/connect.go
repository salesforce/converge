...
// (this broker owns the lease) goes back to its buffer via requeue so another worker
// gets it; a FORWARDED reference (ps.foreign — from a peer, no local lease) is simply
// dropped (the forwarding peer keeps the row; its heartbeat lapses → the reaper
// re-issues it). Returns false when the fan-in goroutine should exit (stop fired).
func forward(ps *pendingStage, merged chan<- *pendingStage, stop <-chan struct{}, f *fanoutExecutor) bool {
	select {
	case merged <- ps:
		return true
	case <-stop:
		if !ps.foreign { // local (this broker owns the lease): return it for another worker
			f.requeue(ps, stop)
		}
		return false
	}
