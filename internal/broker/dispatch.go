...
//
// A FOREIGN task (a reference forwarded by a peer; ps.resultCh == nil) does NOT
// consume a credit slot: this broker holds no lease/resultCh for it, so no local
// resolution path (deliver) would ever free that slot — a slot consumed here would
// leak. Credit is a best-effort routing HINT (correctness is the epoch fence, not the
// slot count), so a forwarded reference merely runs on a worker's real capacity
// without being reflected in the advertised hint. It is always "delivered" (true).
//
