...
func (m *peerMesh) pump(streamCtx context.Context, cancel context.CancelFunc, stream routeBidi, p *peerClient) error {
	...
	// cancel() unblocks the send goroutine; the orphaned Receive goroutine unblocks and
	// exits when the stream is torn down by the deferred cancel in serveRoute/AcceptRoute.
	type recv struct {
