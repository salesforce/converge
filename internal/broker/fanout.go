...
type pendingStage struct {
	...
	// foreign marks a task REFERENCE forwarded to us by a peer (onRouteTask): our local
	// worker runs it, but the peer holds the lease + does the durable fenced write, and
	// we return the StageComplete home over its route. A foreign task consumes the
	// worker's real execution capacity (gateSub) but NOT a credit-HINT slot (slotHeld),
	// and on a stream-teardown requeue it is dropped, not requeued (the peer's reaper
	// re-issues it). This is the SINGLE discriminator for those two decisions
	// (attributeToStream, forward) — a foreign task carries a non-nil resultCh (a
	// forwarder goroutine drains it to send the result home), so resultCh==nil can NOT
	// tell foreign from local; `foreign` does. Set once in onRouteTask, never cleared.
	foreign bool
	...
	// FOREIGN task (ps.foreign — a reference forwarded by a peer; we hold no lease and
	// consume no credit slot for it, though it does carry a resultCh).
	sentBy *configSub
...
// deliver resolves it when our worker Completes) marked foreign, with a resultCh a
// forwarder goroutine drains: on a result it sends the StageComplete home; on our
// ctx/mesh shutdown it just exits (owner's heartbeat-lapse → reaper re-claim is the
// backstop). No credit slot is consumed (we hold no lease); ps.foreign is what tells
// attributeToStream/forward to skip the credit slot + drop-not-requeue on teardown.
func (f *fanoutExecutor) onRouteTask(owner *peerClient, task *workerpb.StageTask) {
	token := task.GetLeaseToken()
	ps := &pendingStage{task: task, resultCh: make(chan *workerpb.StageComplete, 1), foreign: true}
	f.mu.Lock()
	f.inflight[token] = ps
	f.mu.Unlock()
	...
	// owner's reaper re-claim is the backstop. The owning peer is captured here, not looked
	// up per-token, so there is no owner map to maintain.
...
func (f *fanoutExecutor) onPeerGone(peer *peerClient) {
	...
	for _, ps := range f.inflight {
		// pushedTo == peer already selects only stages THIS broker forwarded to the
		// departed peer (a foreign/local-served stage is never pushedTo any peer), so
		// this is not a foreign discriminator; the resultCh nil-guard is pure defense
		// before the send below (every live stage carries a buffered resultCh).
