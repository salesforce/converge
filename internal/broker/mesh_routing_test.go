...
// enqueued on the local kind buffer for a worker to run AND registered in-flight marked
// foreign (with a resultCh a forwarder goroutine drains), so when the local worker
// Completes it the result is RETURNED to the owning peer — captured in the forwarder
// closure — over its route (a `complete` frame); the owner holds the lease + does the
// fenced write. When the buffer is full the reference is dropped (no state left behind) —
// never blocking the route receive loop — and the owner's reaper re-claims it.
func TestOnRouteTask(t *testing.T) {
	t.Run("buffer-has-room-forwards-result-home", func(t *testing.T) {
		...
		// The reference is registered in-flight (so our worker's Complete resolves it),
		// marked foreign, with a resultCh but no local lease (the owner keeps the row).
		f.mu.Lock()
		...
		if !ps.foreign {
			t.Fatal("onRouteTask did not mark the reference foreign")
		}
	...
	t.Run("buffer-full-drops", func(t *testing.T) {
		...
		// Dropped: no local inflight state left behind.
		f.mu.Lock()
