...
func (s *Store) GetResource(ctx context.Context, kind model.Kind, name string) (GetResourceRow, error) {
	return s.queries().GetResource(ctx, dbq.GetResourceParams{Kind: string(kind), Name: name})
}
...
func (s *Store) CreateOperationRow(ctx context.Context, resourceID uuid.UUID, verb string, input []byte, requestedBy string) (ResourceOperation, error) {
	op, err := s.queries().CreateOperation(ctx, dbq.CreateOperationParams{
	...
	if err := s.queries().EnqueueOperationWork(ctx, dbq.EnqueueOperationWorkParams{
		ResourceID: resourceID,
		OpID:       toUUID(op.ID),
	}); err != nil {
		return ResourceOperation{}, err
