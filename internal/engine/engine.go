...
// engineStartUnwindTimeout bounds the best-effort reverse-order Stop of
// already-started duties when a later duty fails to Start (a partial-start abort).
// Short: the pod is aborting boot, so this only needs to let the started sweepers
// stop promptly — not the full graceful DrainGrace the normal Stop path uses.
const engineStartUnwindTimeout = 5 * time.Second
...
// the first duty that fails to start, UNWINDS the duties already started (reverse
// order, best-effort), and returns the error — so a partial-start abort leaves no
// orphaned sweepers running (previously the started duties lingered until the
// caller cancelled the ctx, skipping their ordered drain). Each duty's Start is
// non-blocking (it spawns goroutines and returns), so on success this returns once
// the whole pod is running.
func (e *Engine) Start(ctx context.Context) error {
	for i, d := range e.duties {
		if ds, ok := d.(dispatchSurface); ok {
			e.dispatch = ds
		}
		if err := d.Start(ctx, e.deps); err != nil {
			// Unwind the duties BEFORE i (e.g. the control sweepers started, then the
			// claim duty failed) in reverse start order — the same ordering Stop uses —
			// on a bounded ctx (the caller's ctx may be the one that failed). Stop is
			// idempotent/no-op on a duty that never started, so this is safe.
			stopCtx, cancel := context.WithTimeout(context.Background(), engineStartUnwindTimeout)
			for j := i - 1; j >= 0; j-- {
				_ = e.duties[j].Stop(stopCtx)
			}
			cancel()
			return err
		}
