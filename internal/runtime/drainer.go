...
func (d *Drainer) Run(ctx context.Context) error {
	...
	// is what makes the pool safe for ControlPlane to Close afterwards. (One ctx +
	// one wg here, unlike the dispatcher: the drainer has no graceful-drain window —
	// every goroutine follows ctx and is joined uniformly. The dispatcher splits its
	// goroutines into two groups on two ctxs, ctx vs drainCtx, so its heartbeat
	// outlives its draining tasks; the drainer needs no such split.)
