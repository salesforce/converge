...
//
// DRAIN INVARIANT: the heartbeat runs under drainCtx (NOT ctx), so it keeps refreshing
// the leases of tasks that are still draining after SIGTERM for the WHOLE DrainGrace
// window. A draining task's lease therefore never goes stale to the reaper while the task
// is still running here, so DrainGrace and the reaper's StaleAfter are INDEPENDENT — an
// operator can raise SHUTDOWN_DRAIN above StaleAfter without risking a peer falsely
// reclaiming + re-dispatching a task this pod is still executing (the epoch fence would
// keep that CORRECT, but it would waste the re-execution the drain exists to avoid). Only
// the claim loop and the LISTEN wake stop promptly at ctx cancel (they pull NEW work; the
// listener is a pure latency optimisation with no reason to outlive the claim loop). (Same
// shape as the reactor dispatcher's heartbeat-under-drainCtx invariant.)
func (d *Dispatcher) Run(ctx context.Context) error {
	// In-flight tasks — AND the heartbeat that keeps their leases fresh — run under
	// drainCtx (their ctx parent): it lingers DrainGrace past ctx so running tasks
	// (and a broker's parked dispatchStage awaiting a worker's Complete) finish, with
	// their leases refreshed the whole time, before being cancelled. The claim loop /
	// listener below follow ctx (stop pulling new work at once).
	...
	// TWO goroutine groups with different lifetimes:
	//
	//   taskWG — sweep-launched task goroutines (incl. deadline orphans, see runOne)
	//   and the LISTEN listener. Tasks run under drainCtx (finish through the drain);
	//   the listener runs under ctx (stops pulling wakes at once, it's pure latency).
	//
	//   hbWG — the ONE heartbeat goroutine, run under drainCtx so it keeps refreshing
	//   in-flight leases for the WHOLE drain window (see the DRAIN INVARIANT on Run).
	//
	// The heartbeat must OUTLIVE the tasks (it refreshes their leases as they drain),
	// but must be JOINED before Run returns (no beat may touch the pool after
	// pool.Close). Its loop only exits when drainCtx is cancelled. So on return the
	// required order is: (a) wait out the tasks, (b) cancel drainCtx to stop the
	// heartbeat, (c) join the heartbeat. Defers run LIFO, so the three defers below —
	// PLUS the outer `defer cancelDrain()` registered above — must be REVERSE of that.
	// The ordering is described relative to its neighbours (not absolute ordinals) so
	// inserting a defer between them doesn't silently invalidate the reasoning.
	var taskWG, hbWG sync.WaitGroup

	// Runs AFTER taskWG.Wait below (registered earlier ⇒ runs later): cancel drainCtx
	// so the heartbeat loop returns, then join it — guaranteeing no beat outlives the
	// pool. cancelDrain() is idempotent, so calling it here as well as via the outer
	// `defer cancelDrain()` is harmless; doing it here is what stops the heartbeat
	// before hbWG.Wait() (the outer one runs last, after this join has completed).
	defer func() {
		cancelDrain()
		hbWG.Wait()
	}()

	// Runs BEFORE the cancel-and-join above, AFTER close(d.stopped) below: wait out
	// every task + the listener. Tasks' DB ops run under drainCtx (still live here —
	// it is cancelled only by the defer above), so they drain to completion; the
	// listener's run under ctx (already cancelled) and unblock promptly. The heartbeat
	// is STILL refreshing their leases throughout (it isn't stopped until the defer
	// above runs). The reaper owns reclamation regardless.
	defer taskWG.Wait()

	// Runs FIRST of these defers (registered last): closed the moment Run returns, so
	// a late AddPairLive abandons its send instead of blocking on a buffer no one drains.
	...
	// and its failure is non-fatal. Tracked in taskWG + on ctx so it's signalled
	// and JOINED on shutdown, releasing its held conn before pool.Close. It has no
	// reason to outlive the claim loop (it only feeds NEW-work wakes), so it stops
	// at ctx cancel, not drainCtx.
	taskWG.Add(1)
	go func() {
		defer taskWG.Done()
		d.Listener.Listen(ctx, "dispatcher work_ready", workReadyChannel, d.Trigger)
	}()
	...
	for {
		...
		claimed, err := d.sweep(ctx, drainCtx, &cursor, &taskWG, inFlight)
		if err != nil {
			if ctx.Err() != nil {
				return nil
			}
