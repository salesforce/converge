...
//
// DRAIN INVARIANT: the heartbeat stops at ctx cancel (hbCtx below) while tasks run on
// under drainCtx for up to DrainGrace WITHOUT a lease refresh. That is safe only while
// DrainGrace stays BELOW the reaper's StaleAfter: a task draining past StaleAfter has its
// lease reaped and re-dispatched to a peer while it still runs here. The epoch fence keeps
// the eventual write CORRECT (the stale pod's commit no-ops), so this is wasted
// re-execution during a rolling deploy, never a double-apply — but the whole point of the
// drain is to AVOID that waste, so keep DrainGrace < StaleAfter (the defaults do:
// 45s < the reaper's StaleAfter, and RequireWithin ≤ half of it). If an operator raises
// SHUTDOWN_DRAIN above StaleAfter this margin inverts. (Same shape as the reactor
// dispatcher's heartbeat/LifecycleStaleAfter invariant.)
