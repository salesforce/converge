...
func (d *ReactorDispatcher) Run(ctx context.Context) error {
	// In-flight deliveries — AND the heartbeat that keeps their leases fresh — run
	// under drainCtx: it lingers DrainGrace past ctx so a worker mid-lifecycle-reaction
	// finishes, with its claim refreshed the whole time, before being cancelled. The
	// poll loop / listener below follow ctx (stop claiming at once).
	drainCtx, cancelDrain := drainctx.New(ctx, d.DrainGrace)
	defer cancelDrain()

	wakeup := make(chan struct{}, 1)

	// TWO groups with different lifetimes (mirrors the work Dispatcher): the listener
	// stops at ctx (pure latency wake for NEW work), while the heartbeat runs under
	// drainCtx so it refreshes in-flight leases through the WHOLE drain window. The
	// heartbeat must outlive the deliveries but be joined before Run returns; its loop
	// only exits when drainCtx is cancelled. Deliveries are NOT in either group here —
	// they are launched and joined inside tick's own wg.Wait, which completes before
	// pollLoop (and thus Run) returns. So the only shutdown ordering these defers must
	// enforce is: cancel drainCtx to stop the heartbeat, then join it, before Run
	// returns and the pool closes. Described relative to neighbours (not absolute
	// ordinals) so a future inserted defer can't silently invalidate the reasoning.
	var listenWG, hbWG sync.WaitGroup

	// Runs AFTER listenWG.Wait below (registered earlier ⇒ runs later): cancel drainCtx
	// so the heartbeat loop returns, then join it — no beat outlives the pool.
	// cancelDrain() is idempotent (also deferred above), so calling it here is harmless
	// and is what stops the heartbeat before the join. By the time this runs, pollLoop
	// has already returned, so every delivery has been joined by tick — the heartbeat
	// is the last live goroutine.
	defer func() {
		cancelDrain()
		hbWG.Wait()
	}()

	// Runs BEFORE the cancel-and-join above: join the listener (bound to ctx, already
	// cancelled → unblocks promptly, releasing its held conn).
	defer listenWG.Wait()

	// One held LISTEN conn turns a committed transition into a wake. Pure latency
	// optimisation over the poll: a dead listener just degrades to IdleInterval. Bound
	// to ctx — it only feeds NEW-work wakes, no reason to outlive the claim loop.
	listenWG.Add(1)
	go func() {
		defer listenWG.Done()
	...
	//
	// DRAIN INVARIANT: this loop runs under drainCtx (NOT ctx), so it keeps refreshing an
	// in-flight delivery's claim for the WHOLE DrainGrace window after SIGTERM. A draining
	// delivery's lifecycle_outbox claim therefore never goes stale while it is still running
	// here, so DrainGrace (SHUTDOWN_DRAIN) and LifecycleStaleAfter are INDEPENDENT: raising
	// SHUTDOWN_DRAIN above the stale window can no longer let the reaper re-arm + double-fire
	// a delivery this pod is still executing. (The dedup token remains the backstop against a
	// genuine at-least-once repeat; this removes the shutdown-timing path to one.)
		...
		go func() {
			...
			for {
				...
				case <-t.C:
					...
					// Heartbeat under drainCtx so the refresh still runs during the drain
					// (the whole point of moving off ctx); it ends when drainCtx is cancelled.
					if err := d.Repo.HeartbeatReactorClaims(drainCtx, held, shards); err != nil && drainCtx.Err() == nil {
