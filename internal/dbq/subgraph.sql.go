// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.31.1
// source: subgraph.sql

package dbq

import (
	"context"
	"encoding/json"

	"github.com/google/uuid"
	"github.com/jackc/pgx/v5/pgtype"
	model "github.com/salesforce/converge/internal/model"
)

const subgraphDeps = `-- name: SubgraphDeps :many

SELECT
    dent.kind AS dependent_kind, dent.name AS dependent_name,
    dep.kind  AS dependency_kind, dep.name AS dependency_name
FROM resource_deps d
JOIN resource_meta dent ON dent.id = d.dependent_id
JOIN resource_meta dep  ON dep.id  = d.dependency_id
WHERE d.dependent_id  = ANY($1::uuid[])
  AND d.dependency_id = ANY($1::uuid[])
`

type SubgraphDepsRow struct {
	DependentKind  string `json:"dependent_kind"`
	DependentName  string `json:"dependent_name"`
	DependencyKind string `json:"dependency_kind"`
	DependencyName string `json:"dependency_name"`
}

// Read-side helpers for the resource graph view.
// Edges among the subgraph's node set, addressed by the PUBLIC (kind, name) of
// each endpoint (not the internal id) so the graph joins by ref. Both endpoints
// are already in the node set, so the meta joins are PK probes.
func (q *Queries) SubgraphDeps(ctx context.Context, dollar_1 []uuid.UUID) ([]SubgraphDepsRow, error) {
	rows, err := q.db.Query(ctx, subgraphDeps, dollar_1)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []SubgraphDepsRow
	for rows.Next() {
		var i SubgraphDepsRow
		if err := rows.Scan(
			&i.DependentKind,
			&i.DependentName,
			&i.DependencyKind,
			&i.DependencyName,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const subgraphTransitive = `-- name: SubgraphTransitive :many
WITH RECURSIVE
center(id) AS (
    SELECT sm.id FROM resource_meta sm WHERE sm.kind = $1 AND sm.name = $2
),
ancestors(id, depth) AS (
    SELECT id, 0 FROM center
    UNION
    SELECT d.dependency_id, a.depth + 1
    FROM ancestors a
    JOIN resource_deps d ON d.dependent_id = a.id
    WHERE a.depth < $3::int
),
descendants(id, depth) AS (
    SELECT id, 0 FROM center
    UNION
    SELECT d.dependent_id, dd.depth + 1
    FROM descendants dd
    JOIN resource_deps d ON d.dependency_id = dd.id
    WHERE dd.depth < $4::int
),
all_nodes AS (
    SELECT id, MIN(signed_depth) AS min_depth
    FROM (
        SELECT id, -depth AS signed_depth FROM ancestors
        UNION ALL
        SELECT id, depth AS signed_depth FROM descendants
    ) u
    GROUP BY id
)
SELECT
    r.id, r.kind, m.name, r.owner_id,
    r.generation, r.synced_gen,
    r.is_ready, r.phase,
    r.deletion_requested_at,
    m.labels,
    r.created_at, r.updated_at,
    a.min_depth::int AS min_depth
FROM all_nodes a
JOIN resources r ON r.id = a.id
JOIN resource_meta m ON m.id = r.id
ORDER BY a.min_depth, r.kind, m.name
LIMIT $5::int
`

type SubgraphTransitiveParams struct {
	Kind    string `json:"kind"`
	Name    string `json:"name"`
	Column3 int32  `json:"column_3"`
	Column4 int32  `json:"column_4"`
	Column5 int32  `json:"column_5"`
}

type SubgraphTransitiveRow struct {
	ID                  uuid.UUID          `json:"id"`
	Kind                model.Kind         `json:"kind"`
	Name                string             `json:"name"`
	OwnerID             pgtype.UUID        `json:"owner_id"`
	Generation          int64              `json:"generation"`
	SyncedGen           int64              `json:"synced_gen"`
	IsReady             bool               `json:"is_ready"`
	Phase               string             `json:"phase"`
	DeletionRequestedAt pgtype.Timestamptz `json:"deletion_requested_at"`
	Labels              json.RawMessage    `json:"labels"`
	CreatedAt           pgtype.Timestamptz `json:"created_at"`
	UpdatedAt           pgtype.Timestamptz `json:"updated_at"`
	MinDepth            int32              `json:"min_depth"`
}

// Center the walk on the resource addressed by (kind, name) — resolved in one
// indexed seed subquery, so there's no separate id-resolve round-trip. The two
// depth bounds and the row LIMIT follow.
func (q *Queries) SubgraphTransitive(ctx context.Context, arg SubgraphTransitiveParams) ([]SubgraphTransitiveRow, error) {
	rows, err := q.db.Query(ctx, subgraphTransitive,
		arg.Kind,
		arg.Name,
		arg.Column3,
		arg.Column4,
		arg.Column5,
	)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []SubgraphTransitiveRow
	for rows.Next() {
		var i SubgraphTransitiveRow
		if err := rows.Scan(
			&i.ID,
			&i.Kind,
			&i.Name,
			&i.OwnerID,
			&i.Generation,
			&i.SyncedGen,
			&i.IsReady,
			&i.Phase,
			&i.DeletionRequestedAt,
			&i.Labels,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.MinDepth,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}
